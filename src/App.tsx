...
const ShippingManager: React.FC = () => {
  ...
  const LABEL_FORMATS = {
    zebra_zd420: {
      formats: [
        ...
        { id: 'label_102x76', name: 'Großes Versandetikett', width: 102, height: 76 }
      ]
    }
  };
  ...
  const bulkPrintSelected = () => {
    ...
    setTimeout(() => {
      ...
      alert(`🖨️ Druck-Jobs abgeschlossen!

📊 Gedruckt:
• ${selectedOrders.length} Hardticket-Sets
• ${totalTickets} einzelne Tickets
• ${selectedOrders.length} Lieferscheine

✅ Alle Dokumente erfolgreich gedruckt.`);

      setSelectedOrders([]);
    }, 2000);

    // Öffne Druck-Dialog für Drucker- und Format-Auswahl
  ...
  const executeHardticketShipping = () => {
    ...
    setTimeout(() => {
      ...
      alert(`🚚 Hardticket-Versand erfolgreich!

📊 Versand-Details:
• ${selectedOrders.length} Bestellungen versendet
• ${totalTickets} Hardtickets gedruckt
• Versandart: ${selectedShippingService.name}
• Gesamtkosten: €${totalShippingCost.toFixed(2)}

🖨️ Gedruckte Dokumente:
• Hardtickets (${printLayout})
• Versandetiketten (${selectedLabelFormat})
• Lieferscheine (${deliveryNoteLayout})

📧 Kunden-Benachrichtigungen versendet
✅ Tracking-Nummern generiert`);
      ...
      {showShippingDialog && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200">
              <h3 className="text-2xl font-bold text-gray-900">🚚 Hardticket-Versand konfigurieren</h3>
              <p className="text-gray-600 mt-1">
                {selectedOrders.length} Bestellungen • {orders.filter(o => selectedOrders.includes(o.id)).reduce((sum, o) => sum + o.tickets.length, 0)} Hardtickets
              </p>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <h4 className="text-lg font-semibold text-gray-900 mb-4">📦 Versanddienstleister</h4>
                    ...
                    <label key={service.id} className="flex items-center space-x-3 p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-600">
                          {service.estimatedDays} Werktage • €{service.price}
                          {service.trackingIncluded && ' • 📦 Tracking inklusive'}
                          {service.requiresSignature && ' • ✍️ Unterschrift erforderlich'}
                        </div>
                      </div>
                    </label>
                  ))}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Hardticket-Drucker */}
                <div>
                  <h5 className="font-semibold text-gray-900 mb-3">🎫 Hardticket-Drucker</h5>
                  <select
                ...
                {/* Etikettendrucker */}
                <div>
                  <h5 className="font-semibold text-gray-900 mb-3">📋 Etikettendrucker</h5>
                  <select
                    value={selectedLabelPrinter}
                    onChange={(e) => setSelectedLabelPrinter(e.target.value)}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent mb-3"
                  >
                    <option value="">Etikettendrucker auswählen...</option>
                ...
                <div>
                  <h5 className="font-semibold text-gray-900 mb-3">📦 Lieferschein-Drucker</h5>
                  <div className="p-3 bg-blue-50 rounded-lg border border-blue-200 mb-3">
                    <p className="text-blue-800 text-sm">
                      📋 Verwendet den gleichen Drucker wie Hardtickets
                    </p>
                    <p className="text-blue-700 text-xs mt-1">
                      {selectedPrinter ? printers.find(p => p.id === selectedPrinter)?.name : 'Kein Drucker ausgewählt'}
                    </p>
                  </div>
              ...
              {selectedShippingService && (
                <div className="bg-gray-50 rounded-lg p-4">
                  <h5 className="font-semibold text-gray-900 mb-3">💰 Versandkosten-Übersicht</h5>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="text-center">
                      <div className="text-lg font-bold text-blue-600">€{selectedShippingService.price}</div>
                      <div className="text-sm text-gray-600">Pro Sendung</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold text-emerald-600">
                        €{(selectedShippingService.price * selectedOrders.length).toFixed(2)}
                      </div>
                    ...
                    <div className="text-center">
                      <div className="text-lg font-bold text-orange-600">
                        {selectedShippingService.trackingIncluded ? '✅' : '❌'}
                      </div>
            ...
            <div className="p-6 border-t border-gray-200 flex justify-end space-x-4">
              ...
              >
                ...
                <span>🚚 Hardtickets drucken & versenden</span>
              </button>
            </div>
          </div>
        </div>
